#!/usr/bin/env python3
"""
route.py -- persistent rootless container network, ROUTED (Cilium-style) model.

An alternative to main.py's bridge. Instead of one L2 broadcast domain
(`br-iot`) shared by every container, each container hangs off the `router`
netns by its OWN /32 routed veth, and the router forwards between them at L3.
There is no shared segment, so there is no ARP between containers to poison:
a container only ever ARPs for its gateway, and the router answers. Anti-spoof
is a per-veth `rp_filter` pin rather than the MAC/ARP filtering the bridge model
needs -- see "Why routed" below.

Run exactly like main.py, under PLAIN `podman unshare` (NOT --rootless-netns),
calling the venv interpreter by path (unshare resets the environment):

    podman unshare ./.venv/bin/python route.py up
    podman unshare ./.venv/bin/python route.py verify
    podman unshare ./.venv/bin/python route.py down

Then attach containers by netns path, same as the bridge model:
...
--------
    router netns:  fabric0  10.0.0.1/32     (dummy; the virtual gateway)
                   ...
                   nft table inet fabric: forward-chain flow matrix
...
Why routed (vs. main.py's bridge)
---------------------------------
...
That is the IP anti-spoof pin; MAC is irrelevant at L3, so -- unlike main.py --
we derive no MAC and pin none.

The virtual gateway, and the one deviation from a pure Cilium sketch
--------------------------------------------------------------------
In a full Cilium-style fabric the gateway 10.0.0.1 is *virtual*: it is assigned
to no interface, and proxy_arp answers a container's ARP for it because the
router has a default route (via its uplink) that makes 10.0.0.1 "reachable via
some other interface" -- the precondition for proxy_arp to reply. This model is
deliberately SELF-CONTAINED (no host uplink: in rootless `podman unshare` we sit
in the unowned host netns and cannot create the host end of an uplink veth --
that is a separate, rootful job, exactly why lan-attach.py exists for the bridge
model). With no uplink there is no default route, so proxy_arp would have nothing
to proxy and a purely-virtual gateway would never answer. So we make the gateway
REAL: 10.0.0.1/32 on a `dummy` (fabric0). A local address is answered by the
normal ARP responder (arp_ignore=0) on whichever veth the request arrives on, so
the gateway resolves without an uplink. We still set proxy_arp=1 on each fabric
veth -- it is harmless here (containers only ARP for the gateway) and keeps the
model correct the day an uplink + default route are added.
...
we use it for all of those. But two things are NOT netlink-reachable that way:
  - sysctls live in /proc/sys/net, which reflects the READING PROCESS's netns,
    not whatever ns a socket is bound to; and
  - the nft ruleset is likewise a property of the calling process's netns.
So to set ip_forward / proxy_arp / rp_filter and load the nft table in `router`,
a process must BE in the router netns. We fork a child, os.setns(CLONE_NEWNET)
into router, then write /proc/sys directly and exec `nft -f -` there (see
_run_in_netns). This is the same fork+setns shape lan-attach.py uses; the
difference is that here we already run inside podman's mount namespace (under
`podman unshare`), so the netns bind-mount is already visible -- we need only the
NET hop, not lan-attach's MOUNT hop. Forking (rather than setns in-process) keeps
the parent's netns and its open pyroute2 sockets untouched.
...
# Reuse the model-agnostic netns plumbing from the bridge model: these are the
# shared primitives (paths, ifindex lookups, ns open/create, lo-up), identical
# whether the fabric is bridged or routed. route.py only adds the routed dataplane.
from main import (
    NETNS_DIR,
    ensure_netns,
    find_ifindex,
...
# Owned, persistent router netns. lan-attach.py already targets this name for the
# (rootful) host uplink, so the routed fabric and that future uplink agree on it.
ROUTER = "router"
...
class Host:
    ...
    strict rp_filter), so a derived MAC -- central to main.py's bridge model --
...
def render_nft() -> str:
    ...
    strict on each fabric veth is the spoofing pin.
    """
...
def _run_in_netns(ns_path: str, fn: Callable[[], str]) -> str:
    ...
    bind-mount is already visible (we're in podman's mount ns), unlike
    lan-attach.py which must also enter a MOUNT ns as real root.
