...
- [x] `build_model` — landed as the pure `cmd/turnip/model.go` lowering: config → a fully-resolved `Plan` (not a stateful object graph; see `docs/ARCHITECTURE.md`). `applyPlan` (`apply.go`) drives it
