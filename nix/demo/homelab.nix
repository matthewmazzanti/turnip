# nix/demo/homelab.nix -- the single-file turnip demo: a self-contained,
# bootable NixOS VM that deploys a routed turnip fabric and three podman
# containers (quadlet-nix) onto it. You poke the live containers with `podman
# exec` to SEE the two things turnip does that plain podman can't -- directional
...
#
#   * flows are DIRECTIONAL + default-deny: proxy->hass, proxy->zwave,
#     hass->zwave are allowed; zwave initiates NOTHING (a contained device).
#     podman networks are all-or-nothing per network -- they can't express this.
#     Each container runs an HTTP server answering "hello from <name>", so an
#     allowed hop returns that banner and a denied one times out.
#   * hass additionally hangs a veth into the VM's bridge br0, so it holds a
#     real LAN address (192.168.1.50) and reaches the bridge's LAN IPs directly
#     -- something rootless podman can't give.
#
# This file focuses on the interesting part -- the turnip, quadlet-nix, and
# networkd setup. The uninteresting substrate (rootless podman, console/ssh
# access, VM sizing) lives in ./base.nix, pulled in via `imports`.
# turnipPkg/turnipLib come via specialArgs (flake.nix).
{ pkgs, config, turnipPkg, turnipLib, ... }:
let
  owner = "homelab";
...
in
{
  ...
  # ===========================================
  # The focus: networkd + turnip + quadlet-nix.
  # ===========================================

  # --- networkd: ONE interface, a bridge with one primary + two secondary IPs ------------------
  # The qemu user-mode NIC (eth0) is enslaved to br0; br0 is the only L3
  # interface. The PRIMARY comes from DHCP -- slirp hands out 10.0.2.15 (the
  # :2224->22 hostfwd target, so ssh works) plus a default route + DNS, which
  # the containers need to PULL their image. The two SECONDARY 192.168.1.x are
  # the LAN segment hass's veth link joins -- hass reaches both.
  # RequiredForOnline=routable holds network-online.target (hence the container
  # pulls) until egress is actually up.
  ...
  # turnipLib.turnipService builds the unit from the turnip pkg + the model (a
  # Nix attrset) + the system podman + the owner's uid (it orders after
  # /run/user/<uid>, where the netns + state live). The binary it bakes isn't on
  # anyone's PATH -- the demo is poked via `podman exec`, not a turnip CLI.
  systemd.services.turnip = turnipLib.turnipService {
    turnip = turnipPkg;
    podman = config.virtualisation.podman.package;
    inherit uid;
    # The turnip MODEL. A single routed network `lan`. Every container needs
    # default=true so it can reach a peer's /32 via the gateway (a /32 fabric
    # has no on-link peers -- traffic to a sibling goes up to the router). hass
    # owns its default on eth0 (so it reaches zwave over the fabric); its br0
    # link is a second, NON-default interface (the LAN is reached by the link's
    # connected /24, no default needed).
    config = {
      runtime.user = owner;
      containers = {
        zwave = { };
        hass.links = [
          # The L2 escape: a veth from hass's netns into the VM bridge br0,
          # giving hass a real LAN IP. Outside every router's nft policy -- a
          # deliberate trust grant (see docs/CONFIG-SKETCH.md).
      ...
      networks.lan = {
        ...
        # The real homelab policy. proxy (the ingress reverse-proxy) reaches the
        # hub and the device controller; the hub reaches the device controller;
        # the device controller (zwave) initiates NOTHING -- a compromised IoT
        # device is contained. Directional: each row is one-way (the return
        ...
        flows = [
          ...
          # NOTE: nothing flows TO proxy and zwave initiates nothing --
          # default-deny drops the rest.
        ];
      };
    };
  };
